...
function saveSearchEntry(article, config, isPost) {
  const data = {};
...
hexo.extend.generator.register("json", function (locals) {
  const config = this.config;
  if (!config.search || path.extname(config.search.path) !== ".json") return;

  const database = [];
  const searchfield = config.search.field;

  if (searchfield === "all" || searchfield === "post") {
    filterVisiblePosts(locals.posts).forEach(post => {
      database.push(saveSearchEntry(post, config, true));
    });
  }

  if (searchfield === "all" || searchfield === "page") {
    locals.pages.forEach(page => {
      database.push(saveSearchEntry(page, config, false));
    });
  }
